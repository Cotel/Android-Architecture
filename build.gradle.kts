...
tasks {
    ...
    "dependencyUpdates"(DependencyUpdatesTask::class) {
        resolutionStrategy {
            componentSelection {
                all {
                    // Do not show pre-release version of
                    // library in generated dependency report
                    ...
                    // kAndroid newest version is 0.8.8 (jcenter),
                    // however maven repository contains version 0.8.7 and
...
task("staticCheck") {
    description =
      ...
      Note that this task is intended to run locally (not on CI), 
      because on CI we prefer to have parallel execution and separate reports 
      for each check (multiple statuses eg. on github PR page).
    """.trimMargin()

    group = "verification"
    afterEvaluate {
        // Filter modules with "lintDebug" task (non-Android modules
        // do not have lintDebug task)
        ...
        // By defining Gradle dependency all dependent tasks will
        // run before this "empty" task
